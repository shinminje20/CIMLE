...
def validate(model, z_gen, loader_eval, loss_fn, alpha, current_depth, args):
    ...
    with torch.no_grad():
        for y in tqdm(loader_eval, desc="Generating samples", leave=False, dynamic_ncols=True):
            bs = y.shape[0]
            # codes = z_gen(bs * args.spi , level="all")
            print("bs: ", bs)
            print("args.spi: ", args.spi)
            noise = get_z_gen([(bs,)], bs, sample_method="mixture")
            
            
            outputs = model(noise, 4, alpha=alpha)
            ...
            outputs = outputs.view(bs, 3 + current_depth, args.res[-1], args.res[-1])
    ...
    for current_depth in tqdm(range(start_depth, depth)):
        ...
        for loop in tqdm(range(args.outer_loops),
            ...
            for batch_idx, ys in tqdm(enumerate(loader),
                ...
                # get_codes_in_chunks(iter_data, model, z_gen, loss_fn, num_samples,
                #                     sample_parallelism=self.sample_parallelism, code_bs=self.code_bs)
                # noise = get_codes_in_chunks() 
                print("===============================")
                ...
                generated_samples = model(noise, 3 + current_depth, alpha)
                ...
                ####################################################################
                images_val, lpips_loss_val, mse_loss_val, comb_loss_val = validate(model, z_gen, loader_eval, loss_fn, alpha, current_depth, args)
